...
public class SSHShellTest {
	...
	public void scpRecursive() {
		...
		//shell.recursiveScp(null,null, "C:\\workplace\\server\\apache-flume-1.7.0-bin\\bin", "/usr/local/tmp");
	}
	@Test
	public void scpDir2Remote() {
		SSHShell shell=new SSHShell();
		shell.scpDir2Remote("C:\\workplace\\server\\apache-flume-1.7.0-bin\\bin",new RemoteHost("172.18.6.8",22,"root","12345678.com"), "/usr/local/tmp");	
	}
	@Test
...
//		String startupCmd="nohup "+remoteDir+"/bin/flume-ng  agent --conf "+remoteDir+"/conf  "
...
//		String startupCmd="pwd";
		...
		boolean isSuccess=shell.remoteDeploy("C:\\workplace\\tmp\\linux\\apache-flume-1.7.0-bin",
				new RemoteHost("172.18.6.8",22,"root","12345678.com"), remoteDir, new String[]{chomd777Cmd,startupCmd});
...
//		String startupCmd="pwd";
		...
		System.out.println("执行远程部署命令："+startupCmd);
		boolean isSuccess=shell.remoteDeploy("C:\\workplace\\tmp\\linux\\apache-flume-1.7.0-bin",
				new RemoteHost("172.18.6.8",22,"root","12345678.com"), remoteDir, chomd777Cmd,startupCmd,56781,30000);
		System.out.println(isSuccess);
	}
	@Test
	public void executeShell() {
		...
		String result=shell.execShell(
				new RemoteHost("172.18.6.8",22,"root","12345678.com"), "/usr/local/apache-flume-1.7.0-bin/bin/flume-ng  agent --conf /usr/local/apache-flume-1.7.0-bin/conf  --conf-file /usr/local/apache-flume-1.7.0-bin/conf/filesink.conf  --name a1  -Dflume.root.logger=INFO,console > nohup.out");
	...
	public void executePTYShell() {
		SSHShell shell=new SSHShell();
		///usr/local/apache-flume-1.7.0-bin/bin/flume-ng  agent --conf /usr/local/apache-flume-1.7.0-bin/conf  --conf-file /usr/local/apache-flume-1.7.0-bin/conf/filesink.conf  --name a1  -Dflume.root.logger=INFO,console > nohup.out
		String result=shell.execShellByPTYWithStdout(
				new RemoteHost("172.18.6.8",22,"root","12345678.com"), "nohup /usr/local/apache-flume-1.7.0-bin/bin/flume-ng  agent --conf /usr/local/apache-flume-1.7.0-bin/conf  --conf-file /usr/local/apache-flume-1.7.0-bin/conf/filesink.conf  --name a1  -Dflume.root.logger=INFO,console > nohup.out &\r\n");
		System.out.println(result);
	}
	/**
	 * 鉴权
